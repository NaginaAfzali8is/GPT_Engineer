...
class User:
    def __init__(self, openai_key, project_name, api_cost, project_zip_binary, last_three_messages):
        ...
        # Using timezone-aware datetime object
...
def success():
 ...
 # Retrieve files_dict from URL parameters
    ...
    # Extract content from SystemMessage objects
    messages_to_save = [msg.content for msg in Messages[:-3]]

    # Create a User instance with all necessary data
    new_user = User(Api_Key, project_Name, api_cost,
                    zip_file_binary, messages_to_save)
    inserted_id = new_user.save_to_mongo()
...
def improve_Gen():
    ...
    socketio.emit('files_received', {'files': extracted_files})
...
def improveProject():
    ...
    try:
        ...
        if selected_files:
            ...
            files_dict = FilesDict({file_name: '' for file_name in selected_files})
            ...
            # Save project zip file to MongoDB
            zip_file_binary = zip_buffer.getvalue()
            new_user = User(Api_Key, project_Name, api_cost, zip_file_binary, [])
            inserted_id = new_user.save_to_mongo()
...
def improve_Genn():
    ...
    # Return a dictionary containing file names and their contents
...
def improveProjectt():
    ...
    try:
        ...
        if selected_files:
            # Convert the selected files list to a set for easy comparison
            selected_files_set = set(selected_files)

            # Get the list of files in the original project
            original_files = improve_Gen()

            # Determine the files to be updated and added
            files_to_update = selected_files_set.intersection(original_files)
            new_files = selected_files_set.difference(original_files)

            # Create a FilesDict object to store the updated and new files
            updated_files_dict = FilesDict()

            # Update the content of the existing files
            for filename in files_to_update:
                updated_files_dict[filename] = ''

            # Improve the files using the Agent
            improved_files = Agent.improve(updated_files_dict, Preprompts)
